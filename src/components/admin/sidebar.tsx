...
function SidebarMenu({ name, icon, text, pathname, handleMenu, }: { name: string; icon: React.ReactNode; text?: string; pathname: string; handleMenu?: () => void; }) {
    const [btn, srtBtn] = useState();
    //   const { btn } = useContext(GlobalContext);
    return (
        <button
            className={` ${pathname === `/${name}` ? "text-[--orange] " : "text-[--text-color]"} flex  hover:text-[--orange]  items-center  ${btn ? "justify-center" : "px-4 justify-start w-full gap-[15px]"} py-3`}
            onClick={handleMenu}        >
            <div>{icon}</div>
